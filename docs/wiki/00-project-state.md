...
| Game assets | `public/game-assets/`, `GAME_ASSET_VERSION=23` |

## Текущая версия

- **README:** v1.4 (пользовательская документация)
- **package.json:** 1.0.0
- **GAME_ASSET_VERSION:** 23
...
- **Body Ability Icons Mini-Batch** — **8/12 in-app** (groups 1–2 on disk + skill board); remaining 4 prompt-ready / glyph fallback
- **Body Abilities UI** — RPG skill board layout on `/growth/abilities` (manifest art for 8; glyph fallback for rest)
...
| Следующий visual priority | **Generate remaining Body Ability Icons (4/12)** → process → wire group 3 into skill board |
...
**Batch 2 preparation** (empty-state, plateau artifact, season boss) → **processed + UI wire** → **Body Ability Icons groups 1–2 (8/12) in-app** → generate remaining 4 icons → **Boss Campaign v2** (later).
