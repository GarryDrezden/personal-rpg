...
Dark fantasy карта **9 глав пути**. Трёхслойная desktop-композиция:

| Слой | Компонент | Содержимое |
|------|-----------|------------|
| 1 | `JourneyMapBackground` | CSS `background-size: cover`, декоративный фон |
| 2 | `JourneyPathSvg` | SVG viewBox `1200×520`: путь, узлы, прогресс |
| 3 | `JourneyStageMarker` + `JourneyBossMini` | Компактные маркеры, без 9 больших карточек |

**Detail panel:** `JourneyChapterDetailPanel` под картой — полное описание выбранной/текущей главы.

**Mobile:** `JourneyMapMobile` — вертикальный маршрут, полные `JourneyStageCard` в списке (без absolute).

Координаты узлов — в SVG units (`x`, `y`). Маркеры: `markerPlacement` + clamp в `journeyMapAnchors.ts`.

Опциональные фоны: `public/game-assets/maps/journey-map-bg-desktop.png`, `journey-map-bg-mobile.png` (CSS `background-size: cover`).

Ширина: `/journey` в `AppShell` без `max-w-6xl`; `.journey-map-shell { width: 100% }`.
