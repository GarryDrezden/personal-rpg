...
**Anchors:** `src/constants/journeyMapAnchors.ts`

Dark fantasy карта **9 глав пути**. RPG campaign layout:

| Breakpoint | Layout |
|------------|--------|
| Desktop (≥1024px, ширина >1180px) | Карта слева (~70–75%) + `JourneyChapterDetailPanel` справа (~25–30%) |
| Tablet (1024–1180px) | Карта сверху, detail panel снизу (одна колонка) |
| Mobile (<1024px) | `JourneyMapMobile` — вертикальный маршрут + detail panel под списком |

Трёхслойная desktop-сцена:

| Слой | Компонент | Содержимое |
|------|-----------|------------|
| 1 | `JourneyMapBackground` | CSS `background-size: cover`, Banana-фон + затемняющий overlay |
| 2 | `JourneyPathSvg` | SVG viewBox `1200×520`: путь, узлы, прогресс |
| 3 | `JourneyStagePin` + `JourneyBossMini` | Компактные пины на карте, без 9 больших карточек |

**Detail panel:** `JourneyChapterDetailPanel` — полное описание выбранной/текущей главы (босс, прогресс, «Что двигает путь»).

**Mobile:** `JourneyMapMobile` — вертикальный маршрут, `JourneyStageCard` в списке (без absolute).

Координаты узлов — в SVG units (`x`, `y`). Пины: `pinPlacement` + optional `pinOffset`, clamp в `journeyMapAnchors.ts`. Короткие названия: `shortTitle` в config.

Опциональные фоны: `public/game-assets/maps/journey-map-bg-desktop.png`, `journey-map-bg-mobile.png` (CSS `background-size: cover`, `aspect-ratio: 16/7` на сцене).

Ширина: `/journey` в `AppShell` без `max-w-6xl`; `.journey-map-shell { width: 100% }`. Без `100vw`, negative margins и horizontal scroll.
