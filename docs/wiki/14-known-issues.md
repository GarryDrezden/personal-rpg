...
Closed in hardening v1 + visual UX v1 + page decomposition v1: unauthenticated SQLite API, public `data/*.sqlite`, Cozy free comfort when nutrition is off, Cozy grant/settings split, season window overlap, raw body-stage avatar paths, broken `/dashboard` link, missing error boundary, stale `?v=42` asset tests, male hero manifest PNG vs on-disk WebP, Cozy Journey DF chrome, 768px cramped sidebar, avatar glued to hero floor, giant TodayPage/SettingsPage orchestrators.
