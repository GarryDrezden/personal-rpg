...
|----|----------|------|-------------|--------------|--------|
| KI-01 | Medium | Persistence | Concurrent tabs last-write-wins on `PUT /api/data/:type`. | Two tabs save the same type. | **Closed** for current SPA — `revision` + 409. Old PWA that omits revision is still LWW until SW updates. See [`../audits/data-integrity-v1.md`](../audits/data-integrity-v1.md) |
