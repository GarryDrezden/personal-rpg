...
export function SkillsPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-2xl font-bold">Навыки персонажа</h1>
      ...
      <Card className="bg-gradient-to-br from-amber-50 to-white">
        <div className="grid gap-4 sm:grid-cols-3">
          <div>
            <p className="text-sm text-rpg-muted">Сумма уровней</p>
            <p className="text-2xl font-bold text-gold">{totalLevels}</p>
          </div>
          <div>
            <p className="text-sm text-rpg-muted">Самый прокачанный</p>
            <p className="text-lg font-semibold">
              {hasAnyXp ? `${strongest.icon} ${strongest.title}` : '—'}
            </p>
          </div>
          <div>
            <p className="text-sm text-rpg-muted">Самый слабый</p>
            <p className="text-lg font-semibold">
      ...
      <Card>
        <h2 className="mb-4 text-lg font-semibold">Как качаются навыки</h2>
        <ul className="space-y-2">
          {SKILL_LEVEL_HOW_TO.map(({ skill, actions }) => (
            <li key={skill} className="flex gap-2 text-sm">
              <span className="min-w-[5.5rem] font-semibold text-stone-800">{skill}:</span>
              <span className="text-rpg-muted">{actions}</span>
            </li>
          ))}
        </ul>
      </Card>
