...
export function WeightSpriteProgress({ journey, gender }: WeightSpriteProgressProps) {
  ...
  return (
    <div className="w-full space-y-4">
      <div className="flex justify-center rounded-xl bg-[var(--app-card-strong)] py-4">
        <img
      ...
      <div>
        <div className="mb-2 h-1.5 overflow-hidden rounded-full bg-[var(--app-progress-track)]">
          <div
            className="h-full rounded-full bg-[var(--app-success)] transition-all duration-500"
            style={{
          ...
          {stageImages.map((src, stage) => {
            ...
            return (
              <div key={src} className="flex flex-col items-center gap-1.5">
                <div
                  className={`relative flex h-14 w-full items-end justify-center rounded-lg border-2 bg-[var(--app-card-strong)] p-0.5 transition-all sm:h-16 md:h-20 ${
                    active
                      ? 'border-[var(--app-primary)] shadow-md ring-2 ring-[color-mix(in_srgb,var(--app-primary)_30%,transparent)]'
                      : reached
