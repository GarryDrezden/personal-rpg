    ...
    <Card className="bg-gradient-to-br from-amber-100/60 to-yellow-50">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm text-rpg-muted">Кошелёк наград</p>
            <p className="text-2xl font-bold text-amber-900">
              {availableCoins.toLocaleString('ru')}
            </p>
            <p className="text-xs text-rpg-muted">монет для покупки</p>
            {totalEarned != null && totalSpent != null && (
              <p className="mt-0.5 text-[10px] text-rpg-muted">
                заработано {totalEarned.toLocaleString('ru')} · потрачено {totalSpent.toLocaleString('ru')}
              </p>
            )}
          </div>
        </div>
