...
export const COZY_SEASON_COPY = {
  ...
  intro:
    'Сезон — это 28 дней, в которых дом собирает следы заботы. Не нужно идеальных дней. Достаточно возвращаться и оставлять отметки. В конце сезона остаётся маленькая награда: запись, предмет или тёплая деталь дома.',
  empty:
